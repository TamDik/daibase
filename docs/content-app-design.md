...
キーボードショートカットの現行仕様とプラグインコマンド対応までの計画は `docs/keyboard-shortcuts-plan.md` を参照してください。
...
ヘルプは namespace に依存しない `Special:Help` として提供します。リポジトリの `docs/*.md` はビルド時にアプリへ埋め込み、`Special:Help/{ファイル名}` で読み取り専用表示します。これらはユーザーコンテンツではないため、独自バージョン管理や MCP の編集対象には含めません。
